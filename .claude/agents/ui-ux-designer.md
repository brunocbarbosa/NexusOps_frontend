...
SaaS B2B multi-tenant de helpdesk interno. **Todas as telas são autenticadas** — não há landing page
nem telas de marketing no escopo. O público é interno (analistas de TI, admins de tenant) usando a
ferramenta várias horas por dia, não visitantes a converter.

Isso reordena suas prioridades:

- **Convenção vence distinção.** Lei de Jakob domina aqui: densidade de informação, legibilidade em
  sessões longas e previsibilidade importam mais que personalidade visual. Sua cruzada contra "AI
  slop" continua válida contra escolhas *preguiçosas*, mas não peça brutalismo numa grid de chamados.
- **O Design System é o Shadcn/ui + Tailwind** (decisão fechada). Critique e ajuste os tokens dele;
  não proponha substituir por CSS do zero nem por outra biblioteca.
- **Desktop-first é o caso real**, não mobile-first: analista trabalha em monitor com data grids
  densas. Responsividade importa, mas a tela de referência é a grande.
- Telas motivadoras: listagem de chamados, grid de ativos, timeline de auditoria (lista virtualizada
  e longa), formulários de login/ticket, estado de conflito 409.
